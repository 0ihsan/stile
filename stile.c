...
typedef int I;
         ...
         float mh=714,wr=1.52,factor=0.39328;
               // ===\config/===
...
static void mvwin(I c,I x,I y,I w,I h) { // c:center ws:winsize wp:winpos
...
static CGEventRef handler(CGEventTapProxy p, CGEventType t, CGEventRef event, void* r) {
 I k=CGEventGetIntegerValueField(event,kCGKeyboardEventKeycode), m=(I)CGEventGetFlags(event);
 ...
 if (m==MOD_FN)
  switch (k) { /* always use qwerty layout */
   // t:top l:left r:right h:half q:quarter f:full w:with g:gap c:center r:resize
